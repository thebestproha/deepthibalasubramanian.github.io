...
function generateProfileReadme(config) {
  ...
  const technicalSkills = config.skills.categories.find(cat => cat.name.includes('Technical'))?.items || [];
  const devRelSkills = config.skills.categories.find(cat => cat.name.includes('Developer'))?.items || []; 
